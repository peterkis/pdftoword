...
主要识别对象为试卷、合同、手册和一般文档；杂志、网络期刊不属于项目目标。
在内容准确、主要内容可编辑的前提下，力求还原原布局与排版。
制定需求、选择样本、评测、选型或调整输出策略前，必须读取
`docs/PRODUCT_REQUIREMENTS.md`；不以杂志结果决定核心验收或新增专项能力。
历史评测保留，排除项单列；不得把缩小范围写成旧缺陷已修复。
...
* 后续执行以用户明确激活的当前计划为准，不使用历史 ALL_PHASES 自动推进。
