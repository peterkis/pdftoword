...
## 2026-09-17 — P2W S2-01～06 自动入口与区域融合（待审阅）

- 接入pdf-inspector 1.20.0原生分类/提取，拆分只读预检、几何和Observation。
- 新增auto路由、哈希绑定授权、局部PP/Ovis调用、独立新作业及保守融合；单个表格可局部保图。
- 实际四页扫描、8次模型请求和Pages检查已完成；真实混合文档验收仍待对应输入。
- 1278项测试通过；完整门禁仍有既有私有脚本19个mypy错误。详见 `tasks/reports/P2W-S2-01_TO_06_REPORT.md`。

## 2026-09-09 — T0016 JPG/PNG 数学试卷回归（PARTIAL）

- 新增独立开发期 collector、固定 14 请求协议、等像素输入、视觉标注冻结、分层评分和离线证据校验。
- 真实 run `t0016-mac-20260909T081611Z`：14 次尝试、13 个完整结果；最后 Ovis PNG 建连失败，无重试。
- PP 六次预测一致，Monkey 存在重复波动；关键分块与字符缺陷如实保留，维持 ADR-006 多候选。
- 保存评分修正版本和来源，T0015 只读基线不变；不实现生产识别/转换链路，不执行 T0017。
- 详见 `tasks/reports/T0016_REPORT.md`；尚不建议 ACCEPTED。
