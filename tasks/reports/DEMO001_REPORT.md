...
## PR #3 第四十五轮审核修复（2026-09-13）

无法安全解释的Ovis图片标签变体生成本地源页降级，不访问标签URL。PP图片选项按映射bbox分行并保留模型序列，2×2不再强制一行四列；不规则/乱序布局要求复核。合并图题时解除包含保留块或删除块的展示组。

新增3项缺陷回归先失败后通过及1项图网格几何检查；全仓432 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第四十六轮审核修复（2026-09-13）

任何未被合法IMAGE标签消费的img起始标记（含截断或大小写变体）均进入源页降级。实际复现中示例触发旧异常而非完成正文输出；修复后成功生成审校DOCX。新增回归先失败后通过，全仓433 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第四十七轮审核修复（2026-09-13）
...
## PR #3 第五十轮审核修复（2026-09-13）

逐页可编辑主要内容检查排除页脚/图题/标题、纯选项标签和label_of/caption_of源块；辅助文字不再掩盖正文降级。截断HTML使用通用标签起始检测，不再只维护部分标签清单。

新增7项回归先失败后通过，全仓449 tests passed，ruff、mypy（49 files）及diff通过；谓词接线遗漏由检查发现并补齐后重跑。无新模型调用。

## PR #3 第五十一轮审核修复（2026-09-13）

截断HTML检测结合标准标签名、自定义元素命名和标记上下文，普通a<b/x<y/a<beta比较式保持正文；明确sup/sub等标记仍保留审校降级。

新增3项回归先失败后通过，全仓452 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第五十二轮审核修复（2026-09-13）
...
## PR #3 第五十四轮审核修复（2026-09-13）

通用截断标签限定行首/空白上下文，括号或绝对值左操作数后的比较式保持正文。表格检测不再仅凭行首竖线，而识别分隔行，避免绝对值符号误降级。

新增2项回归先失败后通过，全仓464 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第五十五轮审核修复（2026-09-13）

移动同时解除被交换邻块涉及的文字/图片展示组。近整页图像的文字层完整性无法证明时，输出一次源页降级并保留完整原生字符候选，不丢背景内容也不重复输出整页正文。截断结束标签独立于左侧比较式边界检测。

新增4项回归先失败后通过，全仓468 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。整页背景文字测试改为验证候选保留，不声明该背景页可编辑主内容已完成。

## PR #3 第五十六轮审核修复（2026-09-13）

非inline图片选项格按标签→图片写出，共享图题仍按图片→图题。截断标签上下文增加冒号/逗号/分号，保留先前括号和绝对值比较式保护。

新增3项回归先失败后通过，全仓471 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第五十七轮审核修复（2026-09-13）

公式预检以占位字符保留左操作数上下文，避免公式后比较符误判标签。跨类型文本合并重分类为段落，并隔离原图题/标签关系，保存关系证据与待复核项。

新增3项回归先失败后通过，全仓474 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第五十八轮审核修复（2026-09-13）

已知 HTML/MathML/SVG 截断标签独立于公式左上下文检测；文本合并均隔离原图题/标签关系，避免同类型块把各自范围扩展为整个合并块。原关系保存在证据中并生成审校项。

新增5项回归先失败后通过，全仓479 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第五十九轮审核修复（2026-09-13）

公式位置使用专用占位符，公式后按统一 HTML/MathML/SVG 元素名集合识别截断标签；保留一般文字比较式边界，遇到元素名与变量歧义时保留源页供审校。

新增6项回归先失败后通过，全仓485 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第六十轮审核修复（2026-09-13）

公式后的截断标签支持合法命名空间前缀，按局部元素名执行标准标签检测，带前缀的 MathML/SVG 不再漏过源页降级。

新增3项回归先失败后通过，全仓488 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第六十一轮审核修复（2026-09-13）

非空可编辑标题计入逐页主内容检查，避免标题页误报 DEMO_OUTPUT_INSUFFICIENT；保留对页脚、图题关系和纯标签的排除。

新增1项回归先失败后通过，全仓489 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第六十二轮审核修复（2026-09-13）

标准截断标签检测覆盖中文及英文句末标点上下文，避免正文句末的残片进入可编辑正文。既有比较式回归保持通过。

新增6项回归先失败后通过，全仓495 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。

## PR #3 第六十三轮审核修复（2026-09-13）

原生字符映射异常不再仅依据页面比例记录：受影响行保留源图并建立审校项，原候选保留。统一识别替换符、各平面私用区和非打印字符，NUL不再直接跳过。

新增4项回归先失败后通过，全仓499 tests passed，ruff、mypy（49 files）及diff通过，无新模型调用。
