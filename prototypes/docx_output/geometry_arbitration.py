...
from .common import JOBS, DemoError, crop, digest, new_job, read, safe_path, save
...
def run(
    ...
    for index, page in enumerate(selected["pages"]):
        ...
        if len(eligible) == 1 and eligible[0]["input_sha256"] == digest(job / info["image_path"]):
            entry = eligible[0]
            pp = read(source / f"response-{entry['region_id']}-pp.json")
        ...
        selected["metadata"].setdefault("geometry_support", {})[str(page["page_index"])] = supports
        selected["metadata"].setdefault("source_binding_ledger", {})[str(page["page_index"])] = (
        ...
        if decision.get("content_proof", {}).get("column_order", {}).get("status") == "PROVEN":
            selected["metadata"].setdefault("column_order", {})[str(page["page_index"])] = decision[
                "content_proof"
            ]["column_order"]
    ...
    if shared_structure:
        ...
        finish(job, selected, structure_candidate=shared)
    else:
        finish(job, selected)
