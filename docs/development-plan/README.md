...
## R2-01 工程验证（2026-09-19）

R1-07、R1-08 已分别提交 `9006988`、`a81c4e6`。R2-01 的原生分区/组行/段落候选、
原子账本与首行/悬挂缩进已接入 reconstruction。两个封存电子样本实际生成并用 Word 校对，
1583 项测试成功；全仓既有 2 个临时脚本 mypy 错误独立保留。

详见 [R2-01 报告](../../tasks/reports/P2W-R2-01_REPORT.md)。工程验证范围有限，
完整产品接受仍保留；自然分页、字体、复杂分区与真实几何采用缺口没有被标为已解决。

## R2-02 样式与标题证据（2026-09-19）

R2-01 已提交 `0799af4`。当前分支 `codex/r2-02-style-profile` 已接入字符加权原生字号聚类、
本机字体映射、确定性标题候选与 hash 绑定的零 OCR 样式重导出；显式 profile 支持 Word 基础样式继承。

两个真实样本和一个表格/OMML/混合纸型合成样本已由代理操作 Word 检查。
1604 项测试成功，既有两项临时 mypy 错误独立保留。Word 打印 PDF 的混合自定义纸型差异、
捆绑渲染器中文漏显、Windows 未验证及完整产品接受缺口仍保留。
详见 [R2-02 报告](../../tasks/reports/P2W-R2-02_REPORT.md)。本阶段改动未提交、未推送。

## R2-03 分栏工程验证（2026-09-20）

R2-02 已提交 `ab20f2a`。当前 `codex/r2-03-columns-order` 已实现 band/column 与阅读顺序 DAG、
公共结构前后守恒、Word 原生列/连续分节、完整封印离线重放。
三类合成 DOCX 的离线渲染与来源核验已执行；历史真实样本没有可确认的多栏正向采用。

用户解锁后，三份合成样例已在 Word 16.113.1 执行打开、插字撤销、保存重开及实际 PDF 校对；
18 个来源范围与图片字节保持一致。**R2-03 工程状态为 VERIFIED，范围限已执行检查**，
真实多栏采用和完整产品接受仍保留缺口。未提交、未推送，未进入 R2-04。
详见 [R2-03 报告](../../tasks/reports/P2W-R2-03_REPORT.md)。
