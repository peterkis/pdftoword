...
单元格边距受最小网格列宽四分之一且最多9pt约束；正文有制表符时，默认制表距离不得超过最小可用正文宽度的一半。
已用嵌入字体不被信任或解析，明确不支持；启用noWrap的单元格也明确拒绝，避免内容驱动扩宽。
启用段落bidi、运行rtl或表格bidiVisual时明确不支持，不将XML顺序等同于双向视觉顺序。
未被文字真值描述的运行级非baseline vertAlign明确不支持；结构化OMML上下标仍按公式契约评分。
字号上限为72pt与最小正文短边一半的较小值；表格行宽包含行前/行后预留网格和声明宽度。
内联图片宽度按实际段落缩进及单元格起点、宽度和边距后的剩余范围限制，而非仅用全页宽度。
页面宽高不超过Word实现支持的31680 twip（22英寸），再据此计算正文与图片边界。
