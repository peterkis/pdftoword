# ADR-006：多候选几何权威（Gate B 复审后延续）

状态：**Accepted — Temporary，Gate B reviewed**

日期：2026-08-28

原复审触发点：**T0016 / P0-GATE-001B completion**，已于 2026-09-11 完成复审。

下一复审点：结合 **T0017 及代表性多页、多类型样本**重新评估。复审触发点不等于自动失效。
...
Gate B 复审结论：T0016 已按 `ACCEPTED_WITH_QUALITY_FINDINGS` 完成；单页证据不足以形成永久唯一几何权威，继续保留 PP 与 Monkey 多候选。依据见 [T0016 报告](../tasks/reports/T0016_REPORT.md)。

以下临时权威关系在后续明确决策替代之前继续有效：
...
- 此 ADR 在后续明确决策替代之前继续有效；T0016 完成及后续复审触发均不构成自动失效
- 不得因多候选而增加不必要的模型调用
...
| 1.1.0 | 2026-09-11 | Gate B 已复审，延续多候选至后续明确决策替代；下一复审结合 T0017 与代表性多页、多类型样本，原生 PDF 优先原则不变 |
